...
Metric1D(T=Float64) = Metric1D(zero(T), zero(T))
Metric2D(T=Float64) = Metric2D(zero(T), zero(T), zero(T))
Metric3D(T=Float64) = Metric3D(zero(T), zero(T), zero(T), zero(T))

# the time member defaults to 0, promote all to common datatype
Metric1D(x) = Metric1D(promote(x, 0)...)
Metric2D(x, y) = Metric2D(promote(x, y, 0)...)
Metric3D(x, y, z) = Metric3D(promote(x, y, z, 0)...)
