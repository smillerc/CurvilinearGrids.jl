...
"""
function rotate!(
  grid::AbstractCurvilinearGrid2D, rotation_angle::Quantity{T,NoDims,typeof(Unitful.°)}
) where {T}
...
function rotate!(
  grid::AbstractCurvilinearGrid3D,
  axis_vector::SVector,
  rotation_angle::Quantity{T,NoDims,typeof(Unitful.°)},
) where {T}
  ...
  for c in domain
    ...
    rotated_point .=
      (point .* cos(rotation_angle)) +
      (cross(axis_vector, point) .* sin(rotation_angle)) +
      (axis_vector .* dot(axis_vector, point) .* (1 - cos(rotation_angle)))
